...
void dxb_omx_ews_lock_init(void)
{
	pthread_mutex_init(&lockEWSHandle, NULL);
}

void dxb_omx_ews_lock_deinit(void)
{
	pthread_mutex_destroy(&lockEWSHandle);
}

void dxb_omx_ews_lock(void)
{
	pthread_mutex_lock (&lockEWSHandle);
}

void dxb_omx_ews_unlock(void)
{
	pthread_mutex_unlock (&lockEWSHandle);
}

void dxb_omx_tcc351x_tuner_component_searched_notify(void *pChannelInfo, int searchedCount)
{
...
void dxb_omx_tcc351x_tuner_component_set_channel_notify(int setResult)
{
	omx_tcc351x_tuner_component_PrivateType* omx_tcc351x_tuner_component_Private = (omx_tcc351x_tuner_component_PrivateType*)gTunerOpenmaxStandComp->pComponentPrivate;
	DEBUG(DEFAULT_MESSAGES,"%s %d \n", __func__, setResult);
	OMX_S32 piParam[2];
	piParam[0] = 0;
	piParam[1] = (OMX_S32) setResult;
	(*(omx_tcc351x_tuner_component_Private->callbacks->EventHandler))
		(gTunerOpenmaxStandComp,
		omx_tcc351x_tuner_component_Private->callbackData,
		OMX_EventVendorSpecificEvent, /* The command was completed */
...
void dxb_omx_tcc351x_tuner_component_ews_notify(void *pst_EWS)
{
	int Current_SegNum;
	int Total_SegNum;
	int MessageID;
...
void dxb_omx_tdmbtuner_channel_search_thread(void *param)
{
...
int DXBBOARD_SelRfPath(int dxb_standard)
{
    ...
    switch(dxb_standard)
    {
        case DxB_STANDARD_TDMB:
            flag = DXB_RF_PATH_VHF;
            break;
        case DxB_STANDARD_DVBT:
...
OMX_ERRORTYPE dxb_omx_tcc351x_tuner_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,OMX_STRING cComponentName) 
{
	int err;
	int omxErr;
	...
	dxb_base_audio_port_Constructor(openmaxStandComp, &omx_tcc351x_tuner_component_Private->ports[0], 0, OMX_FALSE);
	pPort = (omx_base_audio_PortType *) omx_tcc351x_tuner_component_Private->ports[OMX_BASE_SOURCE_OUTPUTPORT_INDEX];
	
	omx_tcc351x_tuner_component_Private->BufferMgmtCallback = dxb_omx_tcc351x_tuner_component_BufferMgmtCallback;
	omx_tcc351x_tuner_component_Private->messageHandler = dxb_omx_tcc351x_tuner_component_MessageHandler;	
	omx_tcc351x_tuner_component_Private->destructor = dxb_omx_tcc351x_tuner_component_Destructor;

	openmaxStandComp->SetParameter  = dxb_omx_tcc351x_tuner_component_SetParameter;
	openmaxStandComp->GetParameter  = dxb_omx_tcc351x_tuner_component_GetParameter;

	openmaxStandComp->SetConfig = dxb_omx_tcc351x_tuner_component_SetConfig;
	openmaxStandComp->GetConfig = dxb_omx_tcc351x_tuner_component_GetConfig;

	openmaxStandComp->GetExtensionIndex = dxb_omx_tcc351x_tuner_component_GetExtensionIndex;
...
/** The Destructor 
 */
OMX_ERRORTYPE dxb_omx_tcc351x_tuner_component_Destructor(OMX_COMPONENTTYPE *openmaxStandComp)
{
	omx_tcc351x_tuner_component_PrivateType* omx_tcc351x_tuner_component_Private = openmaxStandComp->pComponentPrivate;
	OMX_U32 i;
...
void dxb_omx_tcc351x_tuner_component_BufferMgmtCallback(OMX_COMPONENTTYPE *openmaxStandComp, OMX_BUFFERHEADERTYPE* outputbuffer) 
{
	OMX_S32 data_read;
	omx_tcc351x_tuner_component_PrivateType* omx_tcc351x_tuner_component_Private = openmaxStandComp->pComponentPrivate;
	int cnt;
...
OMX_ERRORTYPE dxb_omx_tcc351x_tuner_component_GetExtensionIndex(
...
OMX_ERRORTYPE dxb_omx_tcc351x_tuner_component_SetConfig(
  OMX_IN  OMX_HANDLETYPE hComponent,
...
/** setting configurations */
OMX_ERRORTYPE dxb_omx_tcc351x_tuner_component_GetConfig(
  OMX_IN  OMX_HANDLETYPE hComponent,
  OMX_IN  OMX_INDEXTYPE nIndex,
  OMX_IN  OMX_PTR pComponentConfigStructure)
{
...
OMX_ERRORTYPE dxb_omx_tcc351x_tuner_component_SetParameter(
  OMX_IN  OMX_HANDLETYPE hComponent,
	...
	*/
	...
	{		
		...
		case OMX_IndexVendorParamTunerChannelScan:
			if(omx_tcc351x_tuner_component_Private->dxb_standard == DxB_STANDARD_TDMB)
			{
		...
		case OMX_IndexVendorParamTunerChannelSet:		
			if(omx_tcc351x_tuner_component_Private->dxb_standard == DxB_STANDARD_DVBT)
			{
			    //Not Support!!!, USE OMX_IndexVendorParamTunerFrequencySet
				omxErr = OMX_ErrorBadParameter;
			}
			else if(omx_tcc351x_tuner_component_Private->dxb_standard == DxB_STANDARD_ISDBT)
			{
				piArg = (int *)ComponentParameterStructure;
				piArg = (int *)piArg[1];
			...
			else if(omx_tcc351x_tuner_component_Private->dxb_standard == DxB_STANDARD_TDMB)
			{
				piArg = (int *)ComponentParameterStructure;
				...
				if(err == 0)
				{
					...
					dxb_omx_tcc351x_tuner_component_set_channel_notify(err == 0? 1:0);
				}
				else
				{
					omxErr = OMX_ErrorBadParameter;
			...
			else if(omx_tcc351x_tuner_component_Private->dxb_standard == DxB_STANDARD_TDMB)
			{
				...
				else
				{
					...
					{		
						...
						{						
							err = TCCTHREAD_Create(&TCC351XThreadID, (pThreadFunc)tcc351x_tdmb_thread, "tcc351x_tdmb_thread", LOW_PRIORITY_10, NULL);
							if(err)
							{
        ...
        case OMX_IndexVendorParamTunerSetNumberOfBB:		    
            {
                int* piParam = (int*)ComponentParameterStructure;
                int number_of_bb = (int)piParam[1];
...
OMX_ERRORTYPE dxb_omx_tcc351x_tuner_component_GetParameter(
  OMX_IN  OMX_HANDLETYPE hComponent,
  OMX_IN  OMX_INDEXTYPE nParamIndex,
  OMX_INOUT OMX_PTR ComponentParameterStructure)
{
	...
	{		
		case OMX_IndexVendorParamTunerChannelSet:
			{
				...
				pSearchInfo =(TUNER351x_SEARCH_INFO *)piArg[1];
		...
		case OMX_IndexVendorParamTunerChannelSearchStart:				
			if(omx_tcc351x_tuner_component_Private->dxb_standard == DxB_STANDARD_TDMB)
			{
				err = pthread_create(&omx_tcc351x_tuner_component_Private->ChannelSearchThread,
					NULL,
					dxb_omx_tdmbtuner_channel_search_thread,
					openmaxStandComp);
		...
		case OMX_IndexVendorParamGetSignalStrength:
			{
				...
				else if(omx_tcc351x_tuner_component_Private->dxb_standard == DxB_STANDARD_DVBT)
				{
					unsigned int *puiStr = (unsigned int *)piArg[1];
					char *msg = puiStr[4];
		...
		case OMX_IndexVendorParamGetSignalStrengthIndex:
			{
				if(omx_tcc351x_tuner_component_Private->dxb_standard == DxB_STANDARD_ISDBT) {
...
OMX_ERRORTYPE dxb_omx_tcc351x_tuner_component_MessageHandler(OMX_COMPONENTTYPE* openmaxStandComp, internalRequestMessageType *message)
{
...
{
	return (dxb_omx_tcc351x_tuner_component_Constructor(openmaxStandComp, BROADCAST_TCC351X_CSPI_STS_NAME));
}

OMX_ERRORTYPE dxb_omx_tcc351x_CSPI_SPIMS_tuner_component_Init(OMX_COMPONENTTYPE *openmaxStandComp) 
{
	return (dxb_omx_tcc351x_tuner_component_Constructor(openmaxStandComp, BROADCAST_TCC351X_CSPI_SPIMS_NAME));
}

OMX_ERRORTYPE dxb_omx_tcc351x_I2C_STS_tuner_component_Init(OMX_COMPONENTTYPE *openmaxStandComp) 
{
	return (dxb_omx_tcc351x_tuner_component_Constructor(openmaxStandComp, BROADCAST_TCC351X_I2C_STS_NAME));
}

OMX_ERRORTYPE dxb_omx_tcc351x_I2C_SPIMS_tuner_component_Init(OMX_COMPONENTTYPE *openmaxStandComp) 
{
	return (dxb_omx_tcc351x_tuner_component_Constructor(openmaxStandComp, BROADCAST_TCC351X_I2C_SPIMS_NAME));
}

OMX_ERRORTYPE dxb_omx_tcc351x_CSPI_TSIFMOD_tuner_component_Init(OMX_COMPONENTTYPE *openmaxStandComp) 
{
	return (dxb_omx_tcc351x_tuner_component_Constructor(openmaxStandComp, BROADCAST_TCC351X_CSPI_TSIFMOD_NAME));
}

OMX_ERRORTYPE dxb_omx_tcc351x_tuner_component_Deinit(OMX_COMPONENTTYPE *openmaxStandComp)
{
	return dxb_omx_tcc351x_tuner_component_Destructor(openmaxStandComp);
}
