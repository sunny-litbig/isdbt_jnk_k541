...
//				ALOGW("todo. 1. Please insert EWS Processing \n");
				...
				// Sample
				dxb_omx_tcc351x_tuner_component_ews_notify((void*)pFicInfo->psTMC_EWS);
			}
...
int TCC351XTDMB_SearchChannels(void)
{
	...
	{		
				...
				if(TCC351XTDMB_GetState()==STATE_SEARCHED)
				{
					for(j=0;j<giTOTALServiceCount;j++)
					{
						...
						*/
						...
						dxb_omx_tcc351x_tuner_component_searched_notify((void *)&g_tChannelInfo[j], ++iFoundServiceCount);
					}
					break;
				}	
				usleep(5000);
		...
		dxb_omx_tcc351x_tuner_component_searched_notify(NULL, ((i+1)*100/(max_ch+1)));
	}
	dxb_omx_tcc351x_tuner_component_searched_notify(NULL, 0);
			...
			if(TCC351XTDMB_GetState()==STATE_SEARCHED)
			{
				for(serviceIndex = 0; serviceIndex < giTOTALServiceCount; serviceIndex++)
				{
					...
					*/
					...
					dxb_omx_tcc351x_tuner_component_searched_notify((void *)&g_tChannelInfo[serviceIndex], ++iFoundServiceCount);
				}
				break;
			}	
			usleep(5000);
