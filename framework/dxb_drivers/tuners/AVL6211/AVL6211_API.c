...
//static AVL_uint16 g_nDemodTunerArrayIndex = 0;
...
void AVL_DVBSx_Error_Dispose(AVL_DVBSx_ErrorCode r)
{
	switch(r)
	{
		case AVL_DVBSx_EC_OK:
			printk("AVL_DVBSx_EC_OK !\n");
			break;
		case AVL_DVBSx_EC_GeneralFail:
			printk("AVL_DVBSx_EC_GeneralFail !\n");
			break;
		case AVL_DVBSx_EC_I2CFail:
			printk("AVL_DVBSx_EC_I2CFail !\n");
			break;
		case AVL_DVBSx_EC_TimeOut:
			printk("AVL_DVBSx_EC_TimeOut !\n");
			break;
		case AVL_DVBSx_EC_Running:
			printk("AVL_DVBSx_EC_Running !\n");
			break;
		case AVL_DVBSx_EC_InSleepMode:
			printk("AVL_DVBSx_EC_InSleepMode !\n");
			break; 			
		case AVL_DVBSx_EC_MemoryRunout:
			printk("AVL_DVBSx_EC_MemoryRunout !\n");
			break;
		case AVL_DVBSx_EC_BSP_ERROR1:
			printk("AVL_DVBSx_EC_BSP_ERROR1 !\n");
			break;
		case AVL_DVBSx_EC_BSP_ERROR2:
			printk("AVL_DVBSx_EC_BSP_ERROR2 !\n");
			break;
	}
}
...
void AVL_Set_LPF(struct AVL_DVBSx_Chip * pAVLChip, struct AVL_Tuner * pTuner, AVL_uint32 m_uiSymbolRate_Hz)
{	
	struct AVL_Demod_Tuner_Configuration_t *pDemodTunerConfig = &g_DemodTuner_Config[pAVLChip->m_uiBusId];
...
AVL_DVBSx_ErrorCode AVL6211_Initialize(struct AVL_DVBSx_Chip * pAVLChip, struct AVL_Tuner * pTuner, int idx)
{
	struct AVL_DVBSx_Diseqc_Para sDiseqcPara;
	struct AVL_DVBSx_MpegInfo sMpegMode;
	AVL_DVBSx_ErrorCode r = AVL_DVBSx_EC_OK;
	struct AVL_Demod_Tuner_Configuration_t *pDemodTunerConfig = &g_DemodTuner_Config[idx];
	...
	if( AVL_DVBSx_EC_OK != r )
	{
		printk("BSP Initialization failed !\n");
		return (r);
	}
#endif

	//pAVLChip->m_uiBusId=pDemodTunerConfig->m_ChannelId;
	pAVLChip->m_uiBusId=idx;
	...
	if( AVL_DVBSx_EC_OK !=r )
	{
		printk("Chip Object Initialization failed !\n");
		return (r);
	}
	...
	if( AVL_DVBSx_EC_OK !=r )
	{
		printk("IBase Initialization failed !\n");
		return (r);
	}
	...
	printk("Chip Ver:{%d}.{%d}.{%d}  API Ver:{%d}.{%d}.{%d}  Patch Ver:{%d}.{%d}.{%d} \n",
	...
	if(AVL_DVBSx_EC_OK != r)
	{
		printk("Demod Initialization failed !\n");
		return (r);
	}
	...
	if(AVL_DVBSx_EC_OK != r)
	{
		printk("Set RF AGC Polar failed !\n");
		return (r);
	}
	...
	if(AVL_DVBSx_EC_OK != r)
	{
		printk("Set Spectrum Polar failed !\n");
		return (r);
	}
	...
	if(AVL_DVBSx_EC_OK != r)
	{
		printk("Set MPEG output mode failed !\n");
		return (r);
	}
	...
	if(AVL_DVBSx_EC_OK != r)
	{
		printk("Tuner Initialization failed !\n");
		return (r);
	}
	...
	if(AVL_DVBSx_EC_OK != r)
	{
		printk("DiSEqC Initialization failed !\n");
	}
...
AVL_DVBSx_ErrorCode AVL6211_LockChannel(struct AVL_DVBSx_Chip * pAVLChip, struct AVL_Tuner * pTuner, AVL_uint32 tuner_freq, AVL_uint32 signal_symbolrate)
{
	...
	if (AVL_DVBSx_EC_OK != r)
	{
		printk("Reset error status failed !\n");
	}
...
AVL_uint32 AVL6211_GETBer(struct AVL_DVBSx_Chip * pAVLChip)
{
	...
	if (AVL_DVBSx_EC_OK != r)
	{
		printk("Get DVBS BER failed.  This function should only be called if the input signal is a DVBS signal.\n");
	}
	else
	{
	//	printk("BER=%.9f\n",(float)(uiBER*1.0e-9));
	...
	else
	{
		for(i=0; i<47; i++)
		{
			if(uiRFSignalLevel <= SignalLevel[i].SignalLevel)
			{
				...
				{
					printk("RFSignalLevel is too weak !");
				}
				else
				{
...
AVL_uint32 AVL6211_BlindScan_Reset(struct AVL_DVBSx_Chip * pAVLChip, struct AVL_DVBSx_BlindScanAPI_Setting *pBSsetting)
{
	...
	//Default scan rang is from 950 to 2150. User may call this function to change scan frequency rang.
	AVL_DVBSx_IBlindScanAPI_SetFreqRange(pBSsetting, pBSsetting->m_uiScan_Start_Freq_MHz, pBSsetting->m_uiScan_Stop_Freq_MHz);
	AVL_DVBSx_IBlindScanAPI_SetScanMode(pBSsetting, AVL_DVBSx_BS_Fast_Mode);//AVL_DVBSx_BS_Slow_Mode);
	//Default set is AVL_DVBSx_Spectrum_Normal, it must be set correctly according Board HW configuration
	AVL_DVBSx_IBlindScanAPI_SetSpectrumMode(pBSsetting, pDemodTunerConfig->m_Tuner_IQ_SpectrumMode);
	//Set Tuner max LPF value, this value will difference according tuner type
	AVL_DVBSx_IBlindScanAPI_SetMaxLPF(pBSsetting, pDemodTunerConfig->m_uiTunerMaxLPF_100Khz);
