...
static AVL_DVBSx_ErrorCode Tuner_control(UINT32 channel_freq, UINT32 bb_sym,struct AVL_Tuner * pTuner,struct AV2011_Setting * AV2011_Configure);
// I2C write function ( start register, register array pointer, register length)
static AVL_DVBSx_ErrorCode AV2011_I2C_write(UINT8 reg_start,UINT8* buff,UINT8 len,struct AVL_Tuner * pTuner);
...
*****************************************************************************/
AVL_DVBSx_ErrorCode Tuner_control(UINT32 channel_freq, UINT32 tuner_lpf, struct AVL_Tuner * pTuner,struct AV2011_Setting * AV2011_Configure)
{
	UINT8 reg[50];
	UINT32 fracN;
	UINT32 BW;
...
static AVL_DVBSx_ErrorCode AV2011_I2C_write(UINT8 reg_start,UINT8* buff,UINT8 len,struct AVL_Tuner * pTuner)
{
	...
	AVL_uchar ucTemp[50];
	int i;
	AVL_DVBSx_Chip * pAVLChip = pTuner->m_pAVLChip;
	...
	r = AVL_DVBSx_II2CRepeater_SendData((AVL_uchar) pTuner->m_uiSlaveAddress,ucTemp, len+1, pAVLChip );
	if(r != AVL_DVBSx_EC_OK){
		return(r);
	}
	AVL_DVBSx_IBSP_Delay(1);
...
AVL_DVBSx_ErrorCode AVL_DVBSx_ExtAV2011_Lock(  struct AVL_Tuner * pTuner)
{
	...
	if(pTuner->m_pParameters == 0)  //if none
	{
		...
		r = Tuner_control((UINT32)((carrierFrequency_100kHz)/10), (UINT32)((pTuner->m_uiLPF_100kHz)*100), pTuner, &AV2011_Set);
	}
	else  //set AV2011 tuner special setting according to pTuner->m_pParameter
	{
		r = Tuner_control((UINT32)((carrierFrequency_100kHz)/10), (UINT32)((pTuner->m_uiLPF_100kHz)*100), pTuner, (AV2011_Setting *)(pTuner->m_pParameters));
	}
	return(r);
}
