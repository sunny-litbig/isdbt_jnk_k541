...
/* **************************************************** */
/* Multi device configuration */ //troy added, 20130313
/* **************************************************** */

const FrontEndSetup front_end_info[FRONT_END_MAX] = 
{
	{//demod ID 0
		1, //host_i2c_bus_number;
		DMD_I2C_SEL_LOW_V,//demod_saddr_bit; // SADR pin 0V - 0 ;  SADR pin 3.3V - 1 .
		tda18250B, //tuner_type 
		0xc0	, //tuner_slave_address
		DMD_REPEATER_MODE_ON    
	},
	{//demod ID 1
		2, //host_i2c_bus_number;
		DMD_I2C_SEL_LOW_V,//demod_saddr_bit; // SADR pin 0V - 0 ;  SADR pin 3.3V - 1 .
		tda18250B, //tuner_type 
		0xc0	, //tuner_slave_address //just for test !!
		DMD_REPEATER_MODE_ON    
	},		
	{ //demod ID 2 
		0xff, // undefined
		0,
		tuner_type_undefined, //user decide 
		0,
		DMD_REPEATER_MODE_OFF	
	},	  
	{ //demod ID 3 
		0xff, // undefined
		0,  
...
DMD_u32_t	DMD_calc_SQI( DMD_PARAMETER_t* param)
{
	...
	else
	{
      ...
      {
           PRINTF(" --- DVB-T SQI   --- ");
           PRINTF(" cnr = %d,  cnr_rel = %d, bit error =  %d / %d \n ", cnr,cnr_rel,param->info[DMD_E_INFO_BERRNUM],param->info[DMD_E_INFO_BITNUM]);
           PRINTF(" constallation_t = %d, coderate_t= %d \n",mod, cr);
       }
          else if ( param->system == DMD_E_DVBT2 )
       {
           PRINTF(" --- DVB-T2 SQI --- ");
           PRINTF(" cnr = %d, cnr_rel = %d, bit error =  %d / %d \n ", cnr,cnr_rel, param->info[DMD_E_INFO_BERRNUM],param->info[DMD_E_INFO_BITNUM]);
           PRINTF(" constallation_t2 = %d, coderate_t2 = %d \n",mod, cr);
       }		
...
#ifdef COMMON_DEBUG_ON 
	PRINTF(" ---  SQI_done  :  cnr_rel = %d, beri = %d, ber_sqi = %d, sqi = %d --- ",cnr_rel, beri, ber_sqi, sqi/100 ); //20130129
#endif 
	return sqi / CNR_MULT;
}
...
DMD_ERROR_t DMD_read_registers(void)
{
       ...
       {			   
               ...
               PRINTF("DVB-T2 : Regaddr[0x%x]  =  0x%x     \n",i,rd);
       ...
       {              
               ...
               PRINTF("DVB-T Regaddr[0x%x]  =  0x%x     \n",i,rd);
