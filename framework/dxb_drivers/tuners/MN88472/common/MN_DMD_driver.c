...
{
	DMD_ERROR_t	ret;
	DMD_u8_t pseq_status; //Patch is here 
	/* Call pre tune process */
   ...
   ret = DMD_Tuner_tune(  param ); 
		if (ret != DMD_E_OK)
	  { 
	       PRINTF(" Tuner lock signal NG, not go to demod !!!\n") ;  
		  return ret;
	   }
    ...
    if(ret == DMD_E_OK)
    {
        ...
        if (ret == DMD_E_OK && pseq_status == 1)
    	{
            PRINTF("-------------Autoctrl stops !! pls. check load_pseq function !--------");
		    ret = DMD_E_ERROR;
    	}
    }	
	return ret;
...
#else	
	DMD_ERROR_t ret;
	ret = DMD_device_set_info( param , id , val );
	if ((param->system == DMD_E_DVBT2 )&&( id == DMD_E_INFO_DVBT2_SELECTED_PLP)) 
	{ 
...
int Get_RF_T_REF_PWR( DMD_PARAMETER_t * param)
{
	...
	DMD_DVBT_CR_t   coderate;
       ...
       T_REF_PWR = -100;
      	...
      	{
      	         PRINTF(" ERROR : Get_T_REF_PWR, system is not DVB-T @!!\n");
                 return  T_REF_PWR;
      	}
	...
	PRINTF(" GET DVBT REF PWR constellation=%d coderate=%d  , hierarchy_selection = %d( 1 : HP; 0 : LP )\n",constellation,coderate,hierarchy_selection);
       #endif
/*
...
int Get_RF_T2_REF_PWR( DMD_PARAMETER_t * param)
{
      ...
      T2_REF_PWR = -100;
		...
		if ( param->system != DMD_E_DVBT2)
      	{
			PRINTF(" ERROR : Get_T2_REF_PWR, system is not DVB-T2 @!!\n");
			return  T2_REF_PWR;
      	}
