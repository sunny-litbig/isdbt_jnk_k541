...
static BOOL TCSOCK_GetSockOpt (TCSocket * h, int iLevel, int iOptName, void *pOptVal, socklen_t *iOptLen)
{
	return (getsockopt (h->m_hSocket, iLevel, iOptName, pOptVal, iOptLen) == 0) ? TRUE : FALSE;
}
...
TCSocket *NEW_TCSOCK (void)
{
	...
	if (h)
	{
		...
		h->GetSockOpt = TCSOCK_GetSockOpt;
