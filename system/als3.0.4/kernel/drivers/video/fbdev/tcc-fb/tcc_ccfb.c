...
static int tccxxx_ccfb_act_clock(ccfb_dev_config_t *dev, int lcdc_num)
{
	...
	if(lcdc_num == 0){
		dev->pCurLcdc = VIOC_RDMA_GetAddress(VIOC_RDMA01);
		dev->pCurWMix = VIOC_WMIX_GetAddress(VIOC_WMIX0);
	}else{
		dev->pCurLcdc = VIOC_RDMA_GetAddress(VIOC_RDMA05);
		dev->pCurWMix = VIOC_WMIX_GetAddress(VIOC_WMIX1);
	}
